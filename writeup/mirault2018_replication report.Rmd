...
# #### RT LME
# rt_model <- lmer((-1000 / rt) ~ condition + (1|stimulus) + (1|participant),
#                  data = full_data_df_filtered)
# 
# summary(rt_model)
# 
# #### Accuracy GLME
# accuracy_model <- glmer((1 - correct) ~ condition + (1|stimulus) + (1|participant),
...
rt_summary = full_data_df_filtered %>% 
  group_by(condition) %>%
...
rt_plot <- ggplot(rt_summary, aes(x = condition, y = mean_rt, fill = condition)) + 
  ...
  geom_errorbar(aes(ymin = mean_rt - 2 * rt_sem, ymax = mean_rt + 2 * rt_sem), 
                width = .2, position = position_dodge(.9))

acc_summary = full_data_df_filtered %>% 
  group_by(condition) %>%
...
acc_plot <- ggplot(acc_summary, aes(x = condition, y = mean_err, fill = condition)) + 
  geom_bar(position="dodge", stat="identity", width = .6) +
  labs(x="Condition", y="Error rate (%)") +
  scale_fill_manual(values=color_scale) + 
  theme(legend.position = "none") +
  scale_x_discrete(labels=c("TW", "Control", "Grammatical")) +
  geom_errorbar(aes(ymin = mean_err - 2 * err_sem, ymax = mean_err + 2 * err_sem), 
                width = .2, position = position_dodge(.9))

plot_grid(rt_plot, acc_plot)
