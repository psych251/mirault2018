...
#### Age:
summary(full_data_dem$age)
#### Gender:
table(full_data_dem$gender)
```
...
#### RT
plot_grid(orig_rt_plot, rt_plot)
#### Accuracy
plot_grid(orig_acc_plot, acc_plot)
```
...
correl <- as.data.frame(cor(stimuli_filtered))
rownames(correl) <- ea_names
colnames(correl) <- ea_names
kable(correl) %>% kable_styling()

ea_df <- left_join(full_data_df_filtered, stimuli_data, by = c("item_no", "condition")) %>%
  filter(group != 0)

ea_df_ungram <- ea_df %>%
  filter(condition != "gram")

ea_df_correct <- ea_df_ungram %>%
  filter(correct)
...
for (i in 1:3) {
  this_var <- ea_vars[[i]]
  rt_ea_model[[i]] <- lmer((-1000 / rt) ~ (condition * eval(this_var)) + (1|item_no) + (1|participant),
                           data = ea_df_correct, na.action = na.exclude,
                           control = lmerControl(check.conv.singular = .makeCC(action = "ignore", tol = 1e-4)))
  acc_ea_model[[i]] <- glmer((1 - correct) ~ (condition * eval(this_var)) + (1|item_no) + (1|participant),
                             data = ea_df_ungram, family = 'binomial',
                             control = glmerControl(check.conv.singular = 
                                                      .makeCC(action = "ignore", tol = 1e-4)))
  rt_summary_tbl <- as.data.frame(summary(rt_ea_model[[i]])[["coefficients"]]) %>% select(c(1, 2, 3))
  acc_summary_tbl <- as.data.frame(summary(acc_ea_model[[i]])[["coefficients"]]) %>% select(c(1, 2, 3))
  this_summary_tbl <- cbind(rt_summary_tbl, acc_summary_tbl)
  row.names(this_summary_tbl) <- c("(Intercept)", "Condition", ea_names[i], "Interaction")
  print(kable(this_summary_tbl) %>%
          kable_styling() %>%
          add_header_above(c(" " = 1, "RT" = 3, "Error rate" = 3)))
  cat("\n")
}
